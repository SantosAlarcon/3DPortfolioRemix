...
const ProjectCard = ({ name, index, description, tags, image, source_code_link, live_code_link }: ProjectCardProps) => (
    <MotionDiv variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
        ...
        >
            <div className="relative h-[230px] w-full">
                <img src={image} alt={name} className="h-full w-full rounded-2xl object-scale-down" loading="lazy" />
                <div className="card-img_hover absolute inset-0 m-3 flex justify-end gap-2">
                    ...
                    <a
                        ...
                        rel="noreferrer"
                    >
                        <img src={globe} alt="github" className="h-1/2 w-1/2 object-contain" />
                    </a>
                </div>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
                {tags.map((tag: Tag) => (
                    <p key={tag.name} className={`rounded-md px-2 py-[0.15em] text-[14px] ${tag.color}`}>
                        {tag.name}
                    </p>
                ))}
            </div>
    ...
    return (
        <>
            ...
            <div className="flex w-full">
                <MotionP variants={fadeIn("", "", 0.1, 1)} className="mt-3 max-w-3xl text-[18px] leading-[30px] text-secondary">
                    Aquí se reflejarán los proyectos que he ido realizando a lo largo de mi carrera, mostrando mis aptitudes como
                    programador. Cada proyecto está enlazado a su repositorio en GitHub.
                    <br />
                    <br />
                    Pulsa en el icono de <b>GitHub</b> para ir al repositorio, o el icono del <b>globo</b> para ver el proyecto en
                    funcionamiento.
                </MotionP>
            </div>
